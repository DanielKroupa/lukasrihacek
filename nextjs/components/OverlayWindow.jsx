...
export default function OverlayWindow({ isOpen, onClose }) {
  ...
  useEffect(() => {
    ...
    const fetchExperienceInfo = async () => {
      try {
        const { NEXT_PUBLIC_STRAPI_ADDRESS, DEPLOY_STRAPI_ADDRESS } =
          process.env;
        // console.log("NEXT_PUBLIC_STRAPI_ADDRESS:", NEXT_PUBLIC_STRAPI_ADDRESS);
        // console.log("DEPLOY_STRAPI_ADDRESS:", DEPLOY_STRAPI_ADDRESS);
        // console.log("NODE_ENV:", process.env.NODE_ENV);

        const baseUrl =
          process.env.NODE_ENV === "production"
            ? DEPLOY_STRAPI_ADDRESS || "http://localhost:1338"
            : NEXT_PUBLIC_STRAPI_ADDRESS || "http://localhost:1338";
